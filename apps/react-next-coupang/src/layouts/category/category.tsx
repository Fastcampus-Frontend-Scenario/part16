            ...
            <div className={styles.categoryWrapper}>
                <ul className={styles.firstCategory} ref={ref}>
                    {category.menus.map((it, idx) => {
                        ...
                        return (
                           <CategoryMenu isFirst={true} height={height || 0} {...it} key={idx} />
                        )
                    })}
                </ul>
            </div>
...
const CategoryMenu: React.FC<CategoryMenuProps> = (props) => {
    ...
    return (
        <>
            <li className={isFirst ? styles.firstCategoryMenu : styles.nthCategoryMenu} style={{ cursor: url ? 'pointer' : 'default' }}>
                    ...
                    <div className={styles.nthCategoryWrapper} style={{ height: height }}>
                        <ul className={styles.nthCategory}>
                            {depth?.menus?.map((it, idx) => {
                                return (
                                    <CategoryMenu isFirst={false} {...it} height={height} key={idx} />
                                    )
                                })}
                        </ul>
                    </div>
